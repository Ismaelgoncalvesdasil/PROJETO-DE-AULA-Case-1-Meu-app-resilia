...
export async function inserir(req, res) {
  ...
  openDb().then((db) => {
    db.run(
      'INSERT INTO filmes (id,titulo, descricao, porcentagem) VALUES (?,?, ?, ?)',
      [filmes.id, filmes.titulo, filmes.descricao, filmes.porcentagem],
    );
  });
  res.json({
    statusCode: 200,
...
export async function atualizar(req, res) {
  ...
  openDb().then((db) => {
    db.run(
      'UPDATE filmes SET titulo = ?, descricao = ?, porcentagem = ? WHERE id = ?',
      [titulo, descricao, porcentagem, id],
    );
  });
}
